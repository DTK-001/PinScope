...
const arcgisSessionUrl = "https://basemapstyles-api.arcgis.com/arcgis/rest/services/styles/v2/sessions/start?styleFamily=arcgis&f=json";
...
async function sendArcgisSession(request, response) {
  ...
  if (!apiKey) {
    sendJson(response, 503, { error: "ArcGIS API key is not configured on the local server." });
    return;
  }
  try {
    const upstream = await fetch(arcgisSessionUrl, {
